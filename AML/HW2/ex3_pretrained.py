...
#-------------------------------------------------
data_aug_transforms = [torchvision.transforms.ColorJitter(contrast=0.3),
                       torchvision.transforms.RandomHorizontalFlip(p=0.5)]#, transforms.RandomGrayscale(p=0.05)]
###############################################################################
...
class VggModel(nn.Module):
    def __init__(self, n_class, fine_tune, pretrained=True):
        ...
        vgg_11_bn = models.vgg11_bn(weights='IMAGENET1K_V1')
        dim_last_layer = list(vgg_11_bn.modules())[-1].out_features
        l1 = nn.Linear(1000, 100)
        l2 = nn.BatchNorm1d(100)
        l3 = nn.ReLU()
        l4 = nn.Linear(100, n_class)
        #self.layers = nn.Sequential(*list(vgg_11_bn.modules()) + [l1, l2, l3, l4])
        self.layers=nn.Sequential(vgg_11_bn,l1,l2,l3,l4)
...
if fine_tune:
    ...
    params_to_update.append(model.layers[-1].weight)
    params_to_update.append(model.layers[-3].weight)
    params_to_update.append(model.layers[-4].weight)
