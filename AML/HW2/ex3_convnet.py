...
class ConvNet(nn.Module):
    def __init__(self, input_size, hidden_layers, num_classes, norm_layer=None):
        ...
        layers = []
        # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
        m1 = nn.Conv2d(input_size, hidden_size[0],kernel_size = 3, padding=1, stride=1)
        if norm_layer == 'BN':
            m15 = nn.BatchNorm2d(hidden_size[0])

        dropout_layer = torch.nn.Dropout(dropout_prob)  #dropout layer
        ...
        for i  in range(len(hidden_size)-1):
            m1 = nn.Conv2d(hidden_size[i], hidden_size[i+1],kernel_size = 3, padding=1, stride=1)
            if norm_layer == 'BN':
                m15 = nn.BatchNorm2d(hidden_size[i+1])
            m2 = nn.MaxPool2d(2, stride=2)
            m3 = nn.ReLU()
            layers.append(m1)
            layers.append(dropout_layer)
...
def VisualizeFilter(model):
    ...
    kernels = model.layers[0].weight.detach().clone()
